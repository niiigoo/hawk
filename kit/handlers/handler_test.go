...
func TestServerMethsTpl(t *testing.T) {
	const def = `
		syntax = "proto3";
	...
	var he handlerData
	he.Methods = sd.Services[0].Methods
	he.ServiceName = sd.Services[0].Name

	gen, err := applyServerMethsTpl(he)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIsValidFunc(t *testing.T) {
	...
	m := newMethodMap(p.Definition().Services[0].Methods)
	...
	svcName := strings.ToLower(p.Definition().Services[0].Name)
...
func TestPruneDecls(t *testing.T) {
	...
	newDecls := m.pruneDecls(f.Decls, strings.ToLower(p.Definition().Services[0].Name))
...
func TestUpdateMethods(t *testing.T) {
	...
	svc.Methods = []*parser2.Method{allMethods[0]}
...
func renderService(svc *parser2.Service, prev string, data *generic.Data) (string, error) {
